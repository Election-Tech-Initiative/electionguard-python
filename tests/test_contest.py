...
from electionguard.contest import ContestDescription, PlaintextVotedContest, Candidate, encrypt_voted_contest, \
    is_valid_plaintext_voted_contest, is_valid_encrypted_voted_contest, decrypt_voted_contest, make_contest_hash
from electionguard.elgamal import ElGamalKeyPair, elgamal_keypair_from_secret
from electionguard.group import ElementModQ, ONE_MOD_Q, int_to_q, add_q, unwrap_optional, Q, TWO_MOD_P, mult_p
...
class TestContest(unittest.TestCase):
    def test_simple_encryption_decryption_inverses(self):
        ...
        ciphertext = unwrap_optional(encrypt_voted_contest(plaintext, contest, keypair.public_key, nonce))
    ...
    def test_error_conditions(self):
        ...
        plaintext_bad5 = valid_plaintext._replace(contest_hash=add_q(contest_hash, ONE_MOD_Q))
        ...
        keypair = elgamal_keypair_from_secret(int_to_q(2))
        nonce = ONE_MOD_Q
        self.assertRaises(Exception, encrypt_voted_contest, valid_plaintext, keypair.public_key, nonce)

        self.assertIsNone(encrypt_voted_contest(plaintext_bad1, contest, keypair.public_key, nonce))

        # now, we're going to mess with the ciphertext
        c = unwrap_optional(encrypt_voted_contest(valid_plaintext, contest, keypair.public_key, nonce))
        ...
        self.assertFalse(is_valid_encrypted_voted_contest(c3, contest, keypair.public_key))

        c4 = c._replace(contest_hash=add_q(c.contest_hash, ONE_MOD_Q))
        self.assertFalse(is_valid_encrypted_voted_contest(c4, contest, keypair.public_key))
    ...
    @settings(deadline=timedelta(milliseconds=2000), suppress_health_check=[HealthCheck.too_slow], max_examples=10)
    @given(arb_plaintext_voted_contest_well_formed(), arb_elgamal_keypair(), arb_element_mod_q_no_zero())
    def test_encrypt_malformed_plaintext_fails(self, cp: Tuple[ContestDescription, PlaintextVotedContest],
                                               keypair: ElGamalKeyPair, nonce: ElementModQ):
        contest_description, plaintext = cp

        evc = encrypt_voted_contest(plaintext, contest_description, keypair.public_key, nonce,
                                    suppress_validity_check=True)
        self.assertIsNotNone(evc)

        # malformed plaintext with counters out of range should cause elgamal_encrypt to return None
        bad_plaintext = plaintext._replace(choices=list(map(lambda s: s + Q, plaintext.choices)))

        self.assertIsNone(encrypt_voted_contest(bad_plaintext, contest_description, keypair.public_key, nonce,
                                                suppress_validity_check=True))

    @settings(deadline=timedelta(milliseconds=2000), suppress_health_check=[HealthCheck.too_slow], max_examples=10)
    @given(arb_plaintext_voted_contest_well_formed(), arb_elgamal_keypair(), arb_element_mod_q_no_zero())
    def test_encrypt_malformed_ciphertext_fails(self, cp: Tuple[ContestDescription, PlaintextVotedContest],
                                                keypair: ElGamalKeyPair, nonce: ElementModQ):
        contest_description, plaintext = cp
        ...
        evc = encrypt_voted_contest(plaintext, contest_description, keypair.public_key, nonce,
                                    suppress_validity_check=True)
        self.assertIsNotNone(evc)
        self.assertTrue(is_valid_encrypted_voted_contest(evc, contest_description, keypair.public_key))

        # now we tamper with the evc to make sure the verification fails
        bad_selections = evc.encrypted_selections.copy()
        bad_selections[0] = bad_selections[0]._replace(alpha=mult_p(bad_selections[0].alpha, TWO_MOD_P))
        bad_evc = evc._replace(encrypted_selections=bad_selections)
        self.assertFalse(is_valid_encrypted_voted_contest(bad_evc, contest_description, keypair.public_key))
        ...
        self.assertFalse(is_valid_encrypted_voted_contest(bad_evc2, contest_description, keypair.public_key))
        ...
        self.assertFalse(is_valid_encrypted_voted_contest(bad_evc3, contest_description, keypair.public_key))
